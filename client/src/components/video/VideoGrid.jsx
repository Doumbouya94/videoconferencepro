...
// Affichée pendant le partage d'écran dans le coin bas-droit
...
export default function VideoGrid() {
  ...
  // ── SCREEN SHARE ACTIF ────────────────────────────────────
  // On N'affiche PAS screenStream localement → brise la boucle miroir.
  // À la place :
  //   - Les autres participants s'affichent en grille (ils voient notre écran)
  //   - Notre caméra s'affiche en PiP dans le coin
  if (screenStream) {
    return (
      <div className="relative w-full h-full bg-gray-950 flex flex-col">

        {/* Zone principale : participants distants */}
        {participants.length > 0 ? (
          ...
          /* Seul dans la salle pendant le partage */
        ...
        {/* Caméra locale en Picture-in-Picture — coin bas-droit */}
        <PipCamera
          stream={localStream}
          name="Vous"
  ...
  // ── SPOTLIGHT ─────────────────────────────────────────────
  if (layout === 'spotlight') {
    const activePart = participants.find(p => p.socketId === activeSpeakerId);
    const rest       = participants.filter(p => p.socketId !== activeSpeakerId);
    return (
      <div className="flex flex-col h-full gap-1 p-2">
        <div className="flex-1 min-h-0">
          {activePart ? (
            ...
            <VideoTile stream={localStream} name="Vous" isLocal isActive
              muted={!audioEnabled} videoOff={!videoEnabled}
              className="w-full h-full" />
          )}
        </div>
        <div className="flex gap-1 h-24 overflow-x-auto shrink-0">
          {activeSpeakerId !== socket?.id && (
            <VideoTile stream={localStream} name="Vous" isLocal
              muted={!audioEnabled} videoOff={!videoEnabled}
              className="w-36 h-full shrink-0" />
          )}
          {rest.map(p => (
            <VideoTile key={p.socketId}
              stream={remoteStreams.get(p.socketId)} name={p.name}
              isHost={p.socketId === hostId}
              muted={!p.audioEnabled} videoOff={!p.videoEnabled}
              handRaised={p.handRaised}
              className="w-36 h-full shrink-0" />
          ))}
        </div>
  ...
  return (
    <div className={`grid ${gridCols()} gap-1 p-2 h-full auto-rows-fr`}>
      <VideoTile
        stream={localStream} name="Vous" isLocal
